      ...
      <div className="flex flex-col gap-4 sm:flex-col md:flex-row md:justify-between md:items-center">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:gap-6 gap-2 text-center">
          <div className="flex justify-center gap-4 text-gray-700">
            <NavLink to="/jobs" className="hover:text-blue-600 font-medium">
              Jobs
            </NavLink>
            <NavLink to="/trending" className="hover:text-blue-600 font-medium">
              Trending
            </NavLink>
          ...
          {isAuthenticated ? (
            <div className="flex justify-center items-center gap-2">
              {role === "[ADMIN]" && (
                <Admin handleLogout={handleLogout}></Admin>
              )}
              {role === "[JOB_POSTER]" && (
                <JobPosterMenu handleLogout={handleLogout}></JobPosterMenu>
              )}
              {role === "[JOB_SEEKER]" && (
                <AccountMenu handleLogout={handleLogout}></AccountMenu>
              )}
            </div>
          ) : (
